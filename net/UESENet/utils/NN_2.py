...
class train_impl():
    def __init__(self, dataset, bins, num_case, num_cdf_sample,
        ...
        lr=1e-5, weight_decay=1e-6, patience=10, delta=2.0):
    ...
    def K_fold_cross_validation(self, num_fold):
        ...
        for fold in range(num_fold):
            ...
            model = cdf_NN(bins=self.bins, device=self.device, cdf_dim=self.num_cdf_sample)
    ...
    def lr_lambda(self, epoch):
        ...
        if epoch + 1 <= (int)(first_stage):
            return (4.5e-5 / 1e-5) * ((epoch + 1) / first_stage)
        elif (int)(first_stage) < epoch + 1 <= (int)(second_stage):
            decay_rate = 5e-5 / 1e-5
    ...
    def plot_loss(self, train_loss_vec, val_loss_vec):

        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.plot(train_loss_vec, 'r', label='training loss')
        ax.plot(val_loss_vec, 'b--', label='validation loss')
        ax.set_title('Training and Validation Loss of SE-Net')
        ax.set_xlabel('Epoch')
        ax.set_ylabel('Loss')
        ax.legend()
        plt.show()
...
class test_impl():
    def __init__(self, bins, test_data_path, num_case, 
        ...
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.test_model = cdf_NN(bins, device=self.device, cdf_dim=num_sample)
    ...
    def process_data(self, test_model, data):
        ...
        with torch.no_grad():
            ...
            mae = torch.mean((outputs - labels).abs()).item()  # 计算平均绝对差
            mre = torch.mean((outputs - labels).abs() / labels.abs()).item()  # 计算平均绝对差
        ...
        for test_loader in self.test_loader_list:
            for step, data in enumerate(test_loader):
                ...
                if loss == float('inf') or mae == float('inf') or mre == float('inf'):
                    print(f"Error occurred in batch {step}, skipping this batch.")
                    skipped_batches += 1
                    continue
...
class cal_impl():
    def __init__(self, bins, cal_data_path, num_case, 
                 num_sample, num_output, batch_size, load_path,
                 delta=1.0):
        super(cal_impl, self).__init__()

        cal_loader_list = []
        # for data_path in cal_data_path:
        cal_data = util_dataset.cal_cdf_dataset(cal_data_path, var_name='cal_result')
        cal_loader = DataLoader(cal_data, batch_size=batch_size, shuffle=False)
        cal_loader_list.append(cal_loader)
        self.cal_loader_list = cal_loader_list

        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.model = cdf_NN(bins, device=self.device, cdf_dim=num_sample)
    ...
    def cal(self, plot_SE=False):
        ...
        feat = torch.empty(0, dtype=torch.float32, device=self.device)
        result = torch.empty(0, dtype=torch.float32, device=self.device)
        for cal_loader in self.cal_loader_list:
            for _, data in enumerate(cal_loader):
                
                data = data.to(self.device)
                feat = torch.cat((feat, data), dim=0)
                ...
                with torch.no_grad():
                    ...
                    outputs = cal_model(feature)  # 前向传播
                    result = torch.cat((result, outputs), dim=0)

                # visualize curves of cdf
                if plot_SE:
                    self.plot_SE_curve(outputs)

        result = torch.cat((feat, result), dim=1)
        return result.cpu().numpy()

    def plot_SE_curve(self, outputs):
        pass

    def save(self, data, file_name='data', data_type='mat'):
        data_mat = {"SE_data": data}
        savemat(f"{file_name}.{data_type}", data_mat)


class cal():
    def __init__(self, bins, cal_data_path, num_feat, 
                 num_sample, num_output, batch_size, load_path):

        self.cal_impl = cal_impl(bins, cal_data_path, num_feat, 
                        num_sample, num_output, batch_size, load_path)

    def cal(self, plot_SE=False):

        return self.cal_impl.cal(plot_SE)
