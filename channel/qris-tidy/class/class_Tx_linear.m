...
classdef class_Tx_linear < class_Tx
    % Tx-subclass for linear array
    ...
    methods
        function obj = class_Tx_linear(Position, NumberOfAntennas, wavelength, antenn_type, CentralFrequency)
            ...
            Tx_pos_matrix = zeros(size(pos_matrix, 1), 3);
            Tx_pos_matrix(1:1:end, :) = pos_matrix;
            
            % antenna properties
            Tx_antenn = qd_arrayant(antenn_type); % antenna object
            Tx_antenn.center_frequency = CentralFrequency; % center freq
            Tx_antenn.no_elements = NumberOfAntennas; % antenna number
            Tx_antenn.element_position = Tx_pos_matrix';
            for i = 1 : NumberOfAntennas
                Tx_antenn.rotate_pattern(0, 'x', i); % pol1
            end
