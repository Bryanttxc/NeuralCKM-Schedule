...
[BS, AIRS1, AIRS2, UE, Power, General] = gen_phaseOpt_sample(config);
save(sample_path, "BS", "AIRS1", "AIRS2", "UE", "Power", "General");
...
AIRS1.x = config.AIRS_x_set(1);
AIRS1.y = config.AIRS_y_set(1);
AIRS1.M_Y = 16;
AIRS1.M_Z = 16;
...
AIRS2.x = config.AIRS_x_set(2);
AIRS2.y = config.AIRS_y_set(2);
AIRS2.M_Y = 12;
AIRS2.M_Z = 12;
...
AIRS1_dynamicNoise_power = Power.AIRS1_dynamicNoise_power;
AIRS2_dynamicNoise_power = Power.AIRS2_dynamicNoise_power;
...
for elem_idx = 1
    ...
    AIRS1_M = AIRS1.M;
    AIRS2_M = AIRS2.M;
    AIRS1_amplify_power = AIRS1.amplify_power;
    AIRS2_amplify_power = AIRS2.amplify_power;

    for iter = 1:num_large_iter

        fprintf("[Info] Case %d: No.%d channel realization\n", ...
                        elem_idx, iter);
        ...
        for numf = 1:num_small_fading
            ...
            amplify_factor_inst_1 = sqrt( AIRS1_amplify_power ./ ...
                                         (sum(sum_norm_T) + AIRS1_M *AIRS1_dynamicNoise_power) );
            % amplify_factor_inst_1 = sqrt( AIRS_amplify_power ./ ...
            %                             (sum(sum_norm_T) + AIRS_M*num_sc*AIRS1_dynamicNoise_power) ); % Neural CKM needs re-train
            ...
            amplify_factor_inst_2 = sqrt( AIRS2_amplify_power ./ ...
                                         (sum(sum_norm_T) + AIRS2_M * AIRS2_dynamicNoise_power) );
            % amplify_factor_inst_2 = sqrt( AIRS_amplify_power ./ ...
            %                              (sum(sum_norm_T) + AIRS_M*num_sc*AIRS2_dynamicNoise_power) ); % Neural CKM needs re-train
            phi_scatter = exp(-1j*(2*pi*rand(AIRS2_M,1)));
            ...
            % phi_random = exp(-1j*(2*pi*rand(AIRS1_M,1)));
            ...
            noisePart_mat = sum(abs(tmp_r1).^2, 1)' .* amplify_factor_inst_1^2 .* AIRS1_dynamicNoise_power + ...
                            sum(abs(tmp_r2).^2, 1)' .* amplify_factor_inst_2^2 .* AIRS2_dynamicNoise_power + ...
            ...
            mean_noise_power = mean(sum(abs(tmp_r1).^2, 1)) .* amplify_factor_inst_1^2 .* AIRS1_dynamicNoise_power + ...
                               mean(sum(abs(tmp_r2).^2, 1)) .* amplify_factor_inst_2^2 .* AIRS2_dynamicNoise_power + ...
                               noise_power;
