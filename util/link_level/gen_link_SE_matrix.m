...
function [gen_SE_time] = gen_link_SE_matrix(config, num_served)
%GEN_SE_MATRIX generate SE_matrix

%% Preparation

num_IRS = config.num_IRS_vec;
...
for lab = 1:num_lab
    for sub_lab = 1:max_num_sublab
        ...
        full_SE_matrix_path = fullfile(save_data_path, ['SE_matrix_lab_', num2str(lab), '_', num2str(sub_lab), ...
                              '_numUE_', num2str(num_UE), '_numIRS_', num2str(num_IRS), '_', num2str(radius), '.mat']);
