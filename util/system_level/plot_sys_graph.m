...
function plot_sys_graph(input_file, num_IRS, root_path)
...
cnt = 1;
for irs_idx = (3-1)*num_IRS+1:3*num_IRS %1:num_IRS
...
for num_UE_idx = 1:length(UE_sampleMatrix)
    ...
    h = l.visualize(1:l.no_tx, 1:l.no_rx, 2);
    
    hold on
    
    L_line = 50; % length of line
    L_arrow = 50; % length of arrow
    color = [255 128 0]./255;
    linewidth = 5;
    arrow_head_size = 10;
    ...
    for irs_idx = (3-1)*num_IRS+1:3*num_IRS %1:num_IRS
        ...
        % head of arrow
        fill(arrow_head_x, arrow_head_y, color, 'EdgeColor', color, 'MarkerSize', 20, 'HandleVisibility', 'off');
        set(gca, 'fontsize', 35);
    end
